...
function controlCrusoal() {
  ...
  controlPrevBtn.addEventListener("click", () => {
    getPrevImage();
  });
  controlNextBtn.addEventListener("click", () => {
    getNextImage();
  });

  function getPrevImage() {
    if (counter <= 0) counter = backgroundImagesUrls.length - 1;
    else counter--;

    MyCarousel.style.backgroundImage = `url(${backgroundImagesUrls[counter]})`;
  }
  function getNextImage() {
    if (counter >= backgroundImagesUrls.length - 1) counter = 0;
    else counter++;
...
async function createProductsInHtml(productsContainer, products, catName) {
  ...
  products.forEach((product) => {
    ...
    productHeader.className =
      "productHeader w-100 d-flex justify-content-between align-items-center p-2";
    ...
    prodductPriceContainer.className =
      "productPrice d-flex align-items-center p-2 ";
    ...
    currentPriceChildSpan.innerText = `${
    ...
    if (product.discount == 0) oldPriceSpan.classList.add("d-none");
