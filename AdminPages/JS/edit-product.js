...
async function loadProductData(productId) {
  try {
    ...
    document.getElementById("product-quantity").value = product.inStock || 1;
    document.getElementById("product-description").value = product.disc || "";

    // Set category selection
    const categorySelect = document.getElementById("product-category");
    categorySelect.value = product.cat_id || "";
...
async function handleFormSubmit(event) {
  ...
  const formData = {
    ...
    cat_id: document.getElementById("product-category").value,
    discount: parseFloat(document.getElementById("product-discount").value),
    inStock: parseInt(document.getElementById("product-quantity").value),
    disc: document.getElementById("product-description").value.trim(),
  };

  console.log("Form Data:", formData);
  // Validation checks
  if (!formData.name || !formData.cat_id) {
  ...
  try {
    ...
    if (fileInput.files[0]) {
      formData.img = await uploadToImgur(fileInput.files[0]);
    }
