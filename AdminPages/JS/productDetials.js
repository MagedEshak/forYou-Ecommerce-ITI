...
async function displayProductDetails(product, productId) {
  ...
  document.getElementById("productCategory").textContent = product.cat_id;
  document.getElementById("productCategory_id").textContent = product.cat_id;
  document.getElementById("productQuantity").textContent = product.inStock;
  document.getElementById(
    "productDiscount"
  ).textContent = `${product.discount}%`;
  document.getElementById("productDescription").textContent = product.disc;

  //fetch category name from the database
  try {
    const category = await getDocById("Categories", product.cat_id);
